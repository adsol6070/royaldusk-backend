...
const adminAuthController = {
     ...
     */
    initiateAdminRegistration: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
     ...
     */
    completeAdminRegistration: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const { registrationToken, password, name, phone } = req.body;

            if (!registrationToken || !password || !name) {
                throw new ApiError(400, "Registration token, password, and name are required");
            }

            // Verify registration token
            const tokenData = await UserService.verifyRegistrationToken(registrationToken);
            ...
            const { access_token, refresh_token } = await UserService.signTokens(newAdmin);

            // Log admin creation
            console.log(`New admin created: ${newAdmin.email} with role ${newAdmin.role}`);
    ...
    adminLogin: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (![UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.MODERATOR].includes(user.role as UserRole)) {
            ...
            if (user.two_factor_enabled) {
                ...
                const twoFactorToken = await UserService.generateTwoFactorToken(user.id);
    ...
    verify2FA: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            // Verify OTP
            const isValidOTP = await UserService.verifyOTP(user.email, otp, OtpType.TWO_FACTOR);
     ...
     */
    updateAdminRole: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (![UserRole.ADMIN, UserRole.MODERATOR].includes(newRole)) {
                throw new ApiError(400, "Invalid role. Only ADMIN and MODERATOR roles can be assigned");
            }
     ...
     */
    deactivateAdmin: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (!requester || requester.role !== UserRole.SUPER_ADMIN) {
                throw new ApiError(403, "Only super admins can deactivate admin accounts");
            }
            ...
            if (!targetAdmin || ![UserRole.ADMIN, UserRole.MODERATOR].includes(targetAdmin.role as UserRole)) {
    ...
    updateAdminProfile: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            await UserService.logAdminAction({
                ...
                details: { updatedFields: { name: !!name, phone: !!phone, email: !!email } },
                ipAddress: req.ip
            });
    ...
    enableTwoFactorAuth: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
    ...
    disableTwoFactorAuth: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const adminId = req.admin?.id;
            ...
            if (!otp || !password) {
                throw new ApiError(400, "OTP and current password are required to disable 2FA");
            }
            ...
            // Verify OTP
            const isValidOTP = await UserService.verifyOTP(admin.email, otp, OtpType.TWO_FACTOR);
            if (!isValidOTP) {
                throw new ApiError(400, "Invalid or expired OTP");
            }
    ...
    getActivityLogs: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (adminFilter && adminFilter !== adminId) {
                ...
                if (!requester || requester.role !== UserRole.SUPER_ADMIN) {
                    throw new ApiError(403, "Only super admins can view other admin activity logs");
                }
            }
    ...
    refreshAdminTokens: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
    ...
    getSecurityEvents: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (!admin || ![UserRole.SUPER_ADMIN, UserRole.ADMIN].includes(admin.role as UserRole)) {
                throw new ApiError(403, "Insufficient permissions to view security events");
            }

            const events = await UserService.getUnresolvedSecurityEvents(severity as string);
    ...
    resolveSecurityEvent: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (!admin || ![UserRole.SUPER_ADMIN, UserRole.ADMIN].includes(admin.role as UserRole)) {
                throw new ApiError(403, "Insufficient permissions to resolve security events");
            }
